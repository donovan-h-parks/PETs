...
def doWork(args):
	# determine number of clique clusters each node in contained in
	geneIdToShared = {}
	for line in open(args.clusters):
		if 'Node' in line:
			lineSplit = line.split(':')
			geneIds = [x.strip() for x in lineSplit[1].split(',')]
			
			for geneId in geneIds:
				geneIdToShared[geneId] = geneIdToShared.get(geneId, 0) + 1
	...
	x = []
	y = []
	for geneId in geneIdToShared:
		x.append(geneIdToLen[geneId])
		y.append(geneIdToShared[geneId])
	...
	ax.set_ylabel('Shared', fontsize=10)
...
if __name__ == '__main__':
	...
	parser.add_argument('clusters', help='Clique clustering.')
